//=============================================================================
// ECH_CCG.js
// Composite Character Generator 
// Echomap Plugin echomap<echomap@gmail.com>
//=============================================================================

//=============================================================================
/*:
 * @plugindesc v0.10 (Requires ECH_Utils.js) Creates a composite image from a bunch of smaller pieces.
 ...
 *
 * @param writeToDisk
 * @desc write composite image to disk (as faces/writenewcompimage_1)
 * @default true
 * @type boolean
 * @on Write Image file to disk
 * @off Don't Image file to disk
 *
 * @param LogLevel
 ...
 * -- You can use scripts and things to alter the images, but for a more dynamic
 *      view, just use noteags on actors and items, and let the plugin take care
 *      of equips, optimizes, and clear commands.
 *
 * Sprite Type:
 ...
 *
 * Requires folder composite with face and tv and sv subdirectories for the files
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 ...
 * Changelog
 * ============================================================================
 * 2023 09 24 updates to tone
 * 2023 09 18 Reorg code, added, setLayerToActor, clearLayerToActor
...
Imported['ECH - CCG'] = '20230901.1';
Imported.Echomap_CCG = true;
Imported.ECH_CCG = true;
...
if(Echomap.Utils)
	Echomap.Utils.sayHello("CCG",Imported['ECH - CCG']);
...
Echomap.CCG.LogLevel = Number(Echomap.CCG.Parameters['LogLevel'] || 0 );
//-----------------------------------------------------------------------------
...
// * Initialize
//==============================
Echomap.CCG.Game_System_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	Echomap.CCG.Game_System_initialize.call(this);
	//	
	Echomap.Utils.setLogLevel("CCG", Echomap.CCG.LogLevel) //Echomap.LogInfo )
	Echomap.Utils.log2("CCG", "Game_System", "initialize", Echomap.LogDebug, "DebugMsg1", "test2", "test3")
	Echomap.Utils.log2("CCG", "Game_System", "initialize", Echomap.LogInfo,  "InfoMsg1", "test2", "test3")
	//TODO Echomap.CCG.Echomap_Queue.initialize();
	//
	Echomap.CCG.Waiters = new Array();
};
...
Scene_Title.prototype.commandNewGame = function() {
    ...
    Echomap.Utils.log2("CCG", "Scene_Title","commandNewGame", Echomap.LogDebug,'commandNewGame(CCG)', 'commandNewGame');
    if ($gameActors && $gameActors._data) {
        var eProc = new Echomap_LoaderCCG();
        eProc.processNotetagsToGame($gameActors._data);
    }
    Echomap.Utils.log2("CCG", "Scene_Title","commandNewGame", Echomap.LogDebug, 'commandNewGame(CCG)', 'Done');
}
...
//	Echomap.CCG.Scene_Title_start.call(this);
    //Echomap.Utils.log2("CCG", "initialize", "start', Echomap.LogDebug, 'Scene_Title:start','!!!!!!!!!!!!!!!!!');
    /*
...
Scene_Base.prototype.update = function() {
    ...
    if (Echomap.CCG.Echomap_Queue === undefined) {
		//Echomap.Utils.log2("CCG", "Scene_Base", "update", Echomap.LogDebug,'Scene_Equip_onItemOk','No echomap Queue');
        return;
    }
...
Scene_Equip.prototype.onItemOk = function() {
	...
	Echomap.Utils.log2("CCG", "Scene_Equip", "onItemOk", Echomap.LogDebug,'Scene_Equip_onItemOk','!!!!!!!!!!!!!!!!!');
	//Echomap.Utils.log2("CCG", "initialize", Echomap.LogDebug, 'Echomap._loaded_CCG', Echomap._loaded_CCG)
...
Scene_Boot.loadSystemImages = function() {
     ...
     Echomap.Utils.log2("CCG", "Scene_Boot", "loadSystemImages", Echomap.LogDebug,'Scene_Boot:loadSystemImages','!!!!!!!!!!!!!!!!!');
};
/* TODO
...
ImageManager.loadNormalBitmapEcho3 = function(imageLocal) {
	...
	var toneY = imageLocal.toneY();//gray
	...
	*/
    if (!inCache || inCache == undefined) {
        ...
        bitmap.addLoadListener(function() {
			...
			//bitmap.gradientFillRect(0, 0, bitmap.width, bitmap.height, "#AB1232","#67412B",false);
        });
...
ImageManager.loadNormalBitmapEcho2 = function(path, hue, toneR,toneG,toneB,toneY) {
    var key = path + ':' + hue + ':' + toneR+ ':' +toneG+ ':' +toneB;
	var inCache = ImageManager.InCacheEcho(key);
	/*
    var inCache = false;
...
//=============================================================================
// Echomap_Queue
// This object holds processors that need to run.
//  informs callbacks about start and finish of list of jobs
//=============================================================================
function Echomap_Queue() {
    this.initialize.apply(this, arguments);
}
Echomap_Queue.prototype.initialize = function() {
	Echomap.Utils.log2("CCG", "Echomap_Queue", "initialize", Echomap.LogInfo,'Echomap_Queue', 'initialize', "Called" );
    ccgqueue = []
	runningqueue = []
	active = false
	queuefinishedcallouts = []
	queuestartingcallouts = []
};
Echomap_Queue.prototype.queue = function (eProc) {
    Echomap.Utils.log2("CCG", "Echomap_Queue", "initialize", Echomap.LogDebug,'queue', 'requeue', eProc.toString() );
	if(queuestartingcallouts!=undefined){
		queuestartingcallouts.forEach((donecallout) => {
			var bind = eval(donecallout);
			if (bind != ''){
				bind.call(this);
			}
		});
...
/*
Echomap_Queue.prototype.unqueue = function (eProc) {
    Echomap.Utils.log2("CCG", "Echomap_Queue", "initialize", Echomap.LogDebug, 'unqueue', 'unqueue', eProc.toString() );
    ccgqueue.push(eProc);
}
*/
Echomap_Queue.prototype.procIsFinished = function (eProc) {
	runningqueue.pop(eProc)
}
//
Echomap_Queue.prototype.addCallbackFinish = function (eProc) {
	if(!queuefinishedcallouts.includes(eProc))
		queuefinishedcallouts.push(eProc)
}
//
Echomap_Queue.prototype.addCallbackStart = function (eProc) {
	if(!queuestartingcallouts.includes(eProc))
		queuestartingcallouts.push(eProc)
}
//
Echomap_Queue.prototype.queueImageCreateForActor = function (actor,parttype) {
    Echomap.Utils.log2("CCG", "Echomap_Queue", "queueImageCreateForActor", Echomap.LogDebug, 'Called w/actorId: '+actor._actorId+ ' parttype: '+parttype);
    var eProc = new Echomap_Processor(this,actor,parttype);
    Echomap.Utils.log2("CCG", "Echomap_Queue", "queueImageCreateForActor", Echomap.LogDebug, 'queue', eProc.toString() );
    //ccgqueue.push(eProc);
	this.queue(eProc)
    Echomap.Utils.log2("CCG", "Echomap_Queue", "queueImageCreateForActor", Echomap.LogDebug, 'ccgqueue size', ccgqueue.length);
    Echomap.Utils.log2("CCG", "Echomap_Queue", "queueImageCreateForActor", Echomap.LogDebug, 'Done');
}

Echomap_Queue.prototype.processQueue = function (actor,parttype) {
    if(ccgqueue.length>0)
        Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogDebug,'processQueue', 'ccgqueue size', ccgqueue.length);
	//todo shortcircuit if zero?
    var eProc = ccgqueue.pop(); //Echomap_Processor
	runningqueue.pop(eProc)
    if(eProc==undefined) {
        //Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogDebug,'processQueue', 'NO processing' );
		if( active && ccgqueue.length<1 && runningqueue.length<1){
			Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogInfo,"processQueue", "This queue is finished, right?")
			active = false;
			if(queuefinishedcallouts!=undefined){
				queuefinishedcallouts.forEach((donecallout) => {
					var bind = eval(donecallout);
					if (bind != ''){
    ...
    //Check if processing
    if(eProc.isDone()){
        Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogInfo,'processQueue', 'Done processing', eProc.toString() );
        return;
    }
    //check if expired
    if(eProc.isExpired()){
        Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogInfo, 'processQueue', 'Expired processing', eProc.toString() );
        return;
    }
    //run processor
    Echomap.Utils.log2("CCG", "Echomap_Queue", "processQueue", Echomap.LogDebug, 'processQueue', 'processing', eProc.toString() );
	runningqueue.push(eProc)
    eProc.doWork();
...
//=============================================================================
// Echomap_Processor
// This object holds the work to be done and logic to do it
//=============================================================================
...
Echomap_Processor.prototype.initialize = function(queue,actor,parttype) {
	Echomap.Utils.log2("CCG", "Echomap_Processor", "initialize", Echomap.LogInfo, 'Echomap_Processor', 'initialize' );
    var dtn = Date.now();
...
Echomap_Processor.prototype.removeFromList = function () {
    this._manager.procIsFinished(this);
}
Echomap_Processor.prototype.doWork = function () {
    ...
    //Echomap.Utils.log2("CCG", "Echomap_Processor", Echomap.LogDebug, 'doWork', 'Called');
    ...
    if(!this.done) {
		...
		this.removeFromList()
        Echomap.Utils.log2("CCG", "Echomap_Processor", "initialize", Echomap.LogInfo, 'doWork', 'DONE with this eproc', this.toString() );
    }
}
...
Echomap_Processor.prototype.createCompositeImgX = function (basewidth,baseheight, imgLayersA, imgLayersE, imgLayersD, filePath ) {
    ...
    if( imgLayersA == null ) {
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogError, 'createCompositeImgX', 'ERROR: Called but no layers?');
        this.done  = true;
        this.error = true;
        return false;
    }
    ...
    // merge with Equipped
    if( !this.loadedEqX ) {
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'load equip images layer on parttype', this._parttype );
    ...
    if( !this.loadedImgX ) {
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'load images layer on parttype', this._parttype );
    ...
    for (var i = 0, len = imgLayersA.length; i < len; i++) {
          ...
          for (var ji = 0, len = localLayer.length; ji < len; ji++) {
              ...
              if (!bitmap.isReady()) {
				  console.log("Bitmap still loading ")
                  return; //done = false;
              }
    ...
    if(!this.composingImgX && !this.composedImgX) {
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'pre create composite images on parttype', this._parttype );
        this.composingImgX = true;
        this.composingLevel = 0;
        return;
    }
    if(this.composingImgX && !this.composedImgX) {
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'create composite images on parttype', this._parttype );
        this.composedImgX  = false;
        /*
        ...
        */
        ...
        Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'fullCntOfEquiped',fullCntOfEquiped);
    ...
    Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'write file on parttype', this._parttype );
    ...
    Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'cache work on parttype', this._parttype );
	...
	Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'Delete cache', key );
	...
	*/
	...
	if(Echomap.CCGUtil.partytype.face === this._parttype) {
		if(Imported.MOG_ActorHud === true) {
            Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'actor hud on parttype', this._parttype );
    ...
    if( this._parttype === Echomap.CCGUtil.partytype.face ) {
    	Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'setting actorId face', actorId);
		...
		Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'setting actorId tv', actorId);
		...
		Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogDebug, 'createCompositeImgX', 'setting actorId sv', actorId);
		...
		Echomap.Utils.log2("CCG", "Echomap_Processor", "createCompositeImgX", Echomap.LogError, 'createCompositeImgX', 'ERROR!! no such partType!!!', this._parttype);
	}
...
Echomap_Processor.prototype.updateScenes = function(actor) {
	if(SceneManager._scene) {
		...
		Echomap.Utils.log2("CCG", "Echomap_Processor", "updateScenes", Echomap.LogDebug, 'waitForImageLoading', 'SceneManager._scene.constructor=', SceneManager._scene.constructor);
		if( SceneManager._scene.constructor == Scene_Equip ) {
			Echomap.Utils.log2("CCG", "Echomap_Processor", "updateScenes", Echomap.LogDebug, 'waitForImageLoading', 'Update Equip');
			...
			Echomap.Utils.log2("CCG", "Echomap_Processor", "updateScenes", Echomap.LogDebug, 'waitForImageLoading', 'Scene_Boot');
        //} else if( SceneManager._scene.constructor == Scene_Splash ) {
		//	Echomap.Utils.log2("CCG", "Echomap_Processor", Echomap.LogDebug, 'waitForImageLoading', 'Scene_Splash');
		} else if( SceneManager._scene.constructor == Scene_Menu ) {
			Echomap.Utils.log2("CCG", "Echomap_Processor", "updateScenes", Echomap.LogDebug, 'waitForImageLoading', 'Scene_Menu');
		}else {
			Echomap.Utils.log2("CCG", "Echomap_Processor", "updateScenes", Echomap.LogError, 'waitForImageLoading', 'ERROR SCENE UNKNOWN', SceneManager._scene.constructor);
		}
    }//Scene
}
Echomap_Processor.prototype.updateSceneMap = function(actor){
	Echomap.Utils.log2("CCG", "Echomap_Processor", "updateSceneMap", Echomap.LogDebug, 'updateSceneMap', 'Called');
...
Echomap_Processor.prototype.combineImagesLayer = function(bmp,imgLayerA,imgLayerE) {
	...
	for (var i = 0, len = imgLayer.length; i < len; i++) {
		...
		var toneY = imgLocal.toneY();//gray
		Echomap.Utils.log2("CCG", "Echomap_Processor", "combineImagesLayer", Echomap.LogDebug, 'combineImagesLayer', 'dpath', dpath, "Hue:" +hue+ " R:"+toneR+" G:"+toneG+" B:"+toneB);
        ...
        if (!tlayer.isReady()) {
            this.composingImgX = false;
            Echomap.Utils.log2("CCG", "Echomap_Processor", "combineImagesLayer", Echomap.LogWarn, 'combineImagesLayer', 'WARN: bitmap not ready! (WHY?)!!', tlayer.url);
            return; //done = false;
            //throw new Error('Failed to load: ' + tlayer.url);
        }
		//if( toneR!=0 || toneG!=0 || toneB!=0 )
		...
		var w = tlayer.width;
		var h = tlayer.height;
        Echomap.Utils.log2("CCG", "Echomap_Processor", "combineImagesLayer", Echomap.LogDebug, 'combineImagesLayer', 'bitmap ready', tlayer.url);
...
Echomap_Processor.prototype.loadImagesLayer = function(bmp,imgLayer) {
	//Echomap.Utils.log2("CCG", "Echomap_Processor", "loadImagesLayer", Echomap.LogDebug, 'loadImagesLayer', 'Called');
	//Array imgLayer
	for (var i = 0, len = imgLayer.length; i < len; i++) {
		var hue = 0;
		...
		var toneB = imageLocal.toneB();
		if(imageLocal) {
			Echomap.Utils.log2("CCG", "Echomap_Processor", "loadImagesLayer", Echomap.LogDebug, 'loadImagesLayer', 'dpath', dpath, "Hue:" +hue+ " R:"+toneR+" G:"+toneG+" B:"+toneB);
	...
	//Echomap.Utils.log2("CCG", "Echomap_Processor", "loadImagesLayer", Echomap.LogDebug, 'loadImagesLayer', 'Done');
};
...
//=============================================================================
// Echomap_LoaderCCG
// The object class for the Composite image NOTE and image layers processing.
//=============================================================================
...
Echomap_LoaderCCG.prototype.initialize = function() {
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "initialize", Echomap.LogInfo, 'Echomap_LoaderCCG', 'initialize', "Called" );
    if(Echomap.CCG.Echomap_Queue == undefined)
        Echomap.CCG.Echomap_Queue = new Echomap_Queue();
};

Echomap_LoaderCCG.prototype.processNotetagsToGame = function (gameActorsList) {
  Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNotetagsToGame", Echomap.LogDebug, 'processNotetagsToGame', 'Called');
 
  gameActorsList.forEach((gameActor) => {
	var objId   = gameActor._actorId;
	var objName = gameActor._name;
    ...
    if(dataActor){
        ...
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNotetagsToGame", Echomap.LogDebug, 'processNotetagsToGame','ERROR FAILED TO FIND ACTOR', objId);
    }
  });
  Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNotetagsToGame", Echomap.LogDebug, 'processNotetagsToGame','Done');
};

Echomap_LoaderCCG.prototype.processDataToGame = function (gameActor, dataActor) {
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processDataToGame", Echomap.LogDebug, 'processDataToGame', 'Called');
	...
	var objName = gameActor._name;
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processDataToGame", Echomap.LogDebug, 'processDataToGame','objId',   objId);
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processDataToGame", Echomap.LogDebug, 'processDataToGame','objName', objName);
  ...
  Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processDataToGame", Echomap.LogDebug, 'processDataToGame','Done');
};

Echomap_LoaderCCG.prototype.processNoteData = function(gameActor, notedata, baseAsCurrent) {
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'Called' );
	//var objId   = gameActor._actorId;
	...
	if(baseAsCurrent) {
		imgLayersFA = gameActor.composite.layerdata.face.raw;
		imgLayersTV = gameActor.composite.layerdata.tv.raw;
	}
    ...
    for (var i = 0; i < notedata.length; i++) {
      var line = notedata[i];
      Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, "processNoteData", "line", line);
    ...
    if(foundMain) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundMain' );
    ...
    if(foundFAEquip) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundFAEquip' );
        gameActor.composite.layerdata.face.defequip = imgLayersFAe;
    	//Echomap.CCG.layerequipFA[ actorId ] = imgLayersE;
    }
	if(foundTV) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundTV' );
    ...
    if(foundTVEquip) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundTVEquip' );
        gameActor.composite.layerdata.tv.defequip = imgLayersTVe;
    }
	if(foundSV) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundSV' );
    ...
    if(foundSVEquip) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'foundSVEquip' );
        gameActor.composite.layerdata.sv.defequip = imgLayersSVe;
    }


	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processNoteData", Echomap.LogDebug, 'processNoteData', 'Done' );
};

Echomap_LoaderCCG.prototype.parseNoteLineToImgLayers = function(line,imgLayers,parttype,ruledata) {
	// 4:foldefile or face: folder/file
	//eyes: face/female/FG_Eyes_p05_c2"
    var myRegexp1  = "^\\s*([a-zA-Z0-9-_./]+):\\s*([a-zA-Z0-9-_./]+)$" 
	// #:folder/file ...?
    var myRegexp2  = "^\\s*([0-9]+):\\s*([a-zA-Z0-9-_./]+),\\s*([a-zA-Z0-9-_]+)$"
	// acca: sv/female/SV_AccA_p01 0 ??
    var myRegexp2b = "^\\s*([a-z0-9]+):\\s*([a-zA-Z0-9-_./]+),\\s*([a-zA-Z0-9-_]+)$"
	// acca: sv/female/SV_AccA_p01 0 ??
    var myRegexp2c2 = "^\\s*([a-z0-9]+):\\s*([a-zA-Z0-9-_./]+)\\s+([0-9]+)\\s+$"
	//clothing2: armor/FG_Clothing2_p05_c4 0
	// face: folder/file hue
	var myRegexp2c = "^\\s*([a-z0-9]+):\\s*([a-zA-Z0-9-_./]+)\\s+([0-9]+)$"
	//"eyebrows: face/female/FG_Eyebrows_p01_c1_m003 0 -100 -100 -100"
	var myRegexp2d = "^\\s*([a-z0-9]+):\\s*([a-zA-Z0-9-_./]+)\\s+([0-9]+)\\s+([0-9-]+)\\s+([0-9-]+)\\s+([0-9-]+)$"
	var myRegexp2d2 = "^\\s*([a-z0-9]+):\\s*([a-zA-Z0-9-_./]+)\\s+([0-9]+)\\s+([0-9-]+)\\s+([0-9-]+)\\s+([0-9-]+)\\s$"	
	//"21:face/female/FG_FrontHair_p04_c1_m003  0 -100 -100 -100"
    ...
    if( match = line.match(myRegexp2) ) {
		...
		match5 = match[5];
		match6 = match[6];
	} else if( match = line.match(myRegexp2d2) ) {
		var match1a = match[1];
		match1 = Echomap.CCGUtil.parseLayerNameToId(match1a,parttype);
		match2 = match[2];
		match3 = match[3];
    	...
    	//console.log("match1 = " + match1 );
    	//console.log("match2 = " + match2 );
    } else if( match = line.match(myRegexp2c2) ) {
		var match1a = match[1]; // parseLayerNameToId
		match1 = Echomap.CCGUtil.parseLayerNameToId(match1a,parttype);
		match2 = match[2];
		match3 = match[3];
    ...
    if(match && match1 && match1>0 && match1<imgLayers.length && match2.length >0 && match2) {
    	//Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", Echomap.LogDebug, 'processNoteData match2: "',match2+'"');
		//TODO eval? ruleData?
		...
		if(line!=undefined && line.length>1)
			console.warn("Could not find a match for line: <" + line +">")
	}
}

Echomap_LoaderCCG.prototype.processActorEquipped = function (actor) {
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped', 'Called' );
	var actorId = actor._actorId;
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped', 'actorId', actorId);
    if(!actorId) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped', 'ERROR with actorId');
        return;
    }
    if(!actor.composite.layerdata) {
        Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped', 'ERROR with layerdata');
        return;
    }
	...
	if(oequips) {
	  	for (var ii = 0; ii < oequips.length; ii++) {
			...
			if(eobj) {
				...
				Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped eiid: '+eiid+' eobj:"',eobj.name+'"');
				...
				if(charItemFace){
					...
					Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquipped", Echomap.LogDebug, 'processActorEquipped','match',match);
					var line;
					while (match != null) {
				    	line = match[1];
...
Echomap_LoaderCCG.prototype.processActorEquippedWithRules = function (actor, eobj, propt, metadataName, imgLayers, layerType) {
	...
	if( idx> -1 ) {
		Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquippedWithRules", Echomap.LogDebug, 'processActorEquipped','match', metadataName) ;
		...
		if( lastIdxUnder > -1 ) {
			...
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquippedWithRules", Echomap.LogDebug, 'processActorEquipped','ruleData',ruleData);
			...
			var match  = charItemFace.match(myRegexp2);
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquippedWithRules", Echomap.LogDebug, 'processActorEquipped','match',match);
			var line;
			while (match != null) {
		    	line = match[1];
				//eval? ruleData ? TODO
				...
				if(ruleResult)
					...
					Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "processActorEquippedWithRules", Echomap.LogDebug, 'processActorEquipped','not parsing per failed rule', ruleData, 0);
				charItemFace = charItemFace.replace( line, "" );
				match = charItemFace.match(myRegexp2);
			}
		}
...
Echomap_LoaderCCG.prototype.setLayerToActor = function( params ) {
	var actorId = params.get("actorId");
	var layerName = params.get("layerName");
	var fileName = params.get("fileName");
	var layerType = params.get("layerType");
	var extradata = params.get("extradata");
	...
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "setLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'layerId', layerId );
	if(layerId && layerId > 0 && layerId < Echomap.CCGUtil.layerMax) {
		...
		if(imgLayers){
			...
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "setLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'index', index);
			if (index < 0 ) {
				//this.parseNoteLineToImgLayers(layerId+":"+fileName,imgLayers,layerType);
				this.parseNoteLineToImgLayers(layerId+":"+fileName + extradata ,imgLayers,layerType);
				Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "setLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'added image to layer', layerId );
			} else
				Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "setLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'Found object already there',0);
		} else {
			//Echomap.Utils.log2("CCG", "initialize", Echomap.LogDebug, 'addLayerToActor', 'Error: No layers to add to',0);
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "setLayerToActor", Echomap.LogError, 'addLayerToActor', 'Error: No layers to add to',0);
		}
	}
};

Echomap_LoaderCCG.prototype.addLayerToActor = function(actorId,layerName,fileName,layerType,extradata) {
	var layerId;
	if(layerType!=undefined){
		layerId = Echomap.CCGUtil.parseLayerNameToId(layerName,layerType);
	} else {
	...
	if(extradata==undefined)
		...
		extradata = " " + extradata
    var gameActor = $gameActors.actor(actorId)
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "addLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'layerId', layerId );
	if(layerId && layerId > 0 && layerId < Echomap.CCGUtil.layerMax) {
		var imgLayers;
		if(layerType== Echomap.CCGUtil.partytype.face)
            imgLayers = gameActor.composite.layerdata.face.raw;
		...
		if(imgLayers){
			...
			//var index = imgLayer.indexOf(fileName);
			var index = Echomap.CCGUtil.findIndexOfImgObject(imgLayer,fileName);
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "addLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'index', index);
			if (index < 0 ) {
				//this.parseNoteLineToImgLayers(layerId+":"+fileName,imgLayers,layerType);
				this.parseNoteLineToImgLayers(layerId+":"+fileName + extradata ,imgLayers,layerType);
				Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "addLayerToActor", Echomap.LogDebug, 'addLayerToActor', 'added image to layer', layerId );
			} else
				console.warn('addLayerToActor', 'Found object already there', 0 );
		} else
			console.warn('addLayerToActor', 'Error: No layers to add to',0);
	}
};
...
Echomap_LoaderCCG.prototype.remLayerFromActor = function(actorId,layerName,fileName,layerType) {
    ...
    var gameActor = $gameActors.actor(actorId)
	if(layerId && layerId>0 && layerId<Echomap.CCGUtil.layerMax) {
		Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "remLayerFromActor", Echomap.LogDebug, 'remLayerFromActor', 'layerId', layerId );
		var imgLayers;
		if(layerType== Echomap.CCGUtil.partytype.face)
            imgLayers = gameActor.composite.layerdata.face.raw;
        else if(layerType== Echomap.CCGUtil.partytype.tv )
		...
		if(imgLayers){
			...
			if(fileName) {
				...
				if (index > -1) {
					Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "remLayerFromActor", Echomap.LogDebug, 'remLayerFromActor', 'fileName', fileName );
				    imgLayer = imgLayer.splice(index, 1);
				}
			} else {
				imgLayers[layerId] = new Array();
				Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "remLayerFromActor", Echomap.LogDebug, 'remLayerFromActor', 'Clean entire layerId', layerId );
			}
		}
	}
};

Echomap_LoaderCCG.prototype.clearLayerFromActor = function(actorId,layerName,layerType) {
	var layerId;
	if(layerType!=undefined){
		layerId = Echomap.CCGUtil.parseLayerNameToId(layerName,layerType);
	} else {
    ...
    var gameActor = $gameActors.actor(actorId)
	if(layerId && layerId>0 && layerId<Echomap.CCGUtil.layerMax) {
		Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "clearLayerFromActor", Echomap.LogDebug, 'remLayerFromActor', 'layerId', layerId );
		var imgLayers;
		if(layerType== Echomap.CCGUtil.partytype.face)
            imgLayers = gameActor.composite.layerdata.face.raw;
        else if(layerType== Echomap.CCGUtil.partytype.tv )
		...
		if(imgLayers){
			imgLayers[layerId] = new Array();
			Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "clearLayerFromActor", Echomap.LogDebug, 'remLayerFromActor', 'Clean entire layerId', layerId );
		}
	}
};

Echomap_LoaderCCG.prototype.hasBackupLayers = function(actor,backupName){
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "hasBackupLayers", Echomap.LogDebug, 'hasBackupLayers', 'Called');
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "hasBackupLayers", Echomap.LogDebug, 'hasBackupLayers', 'backupName', backupName);
    var retVal  = true;
    if(!actor.composite)
        retVal  = false;
    ...
    if(retVal && !actor.composite.layerdata.backup[backupName].face.raw)
        retVal  = false;
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "hasBackupLayers", Echomap.LogDebug, 'hasBackupLayers', 'retVal', retVal);
    return retVal;
}
Echomap_LoaderCCG.prototype.makeBackupLayers = function(actor,backupName){
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "makeBackupLayers", Echomap.LogDebug, 'makeBackupLayers', 'Called');
    if(!actor.composite.layerdata)
        return;
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "makeBackupLayers", Echomap.LogDebug, 'makeBackupLayers', 'start backupName',backupName);
    ...
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "makeBackupLayers", Echomap.LogDebug, 'makeBackupLayers', 'done  backupName',backupName);
}

Echomap_LoaderCCG.prototype.restoreBackupLayers = function(actor,backupName, backupCurrent,backupCurrentName){
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "restoreBackupLayers", Echomap.LogDebug, 'restoreBackupLayers', 'Called');
    if(!actor.composite.layerdata)
        return;
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "restoreBackupLayers", Echomap.LogDebug, 'restoreBackupLayers', 'start backupName',backupName);
    ...
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "restoreBackupLayers", Echomap.LogDebug, 'restoreBackupLayers', 'done backupName',backupName);
    ...
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "restoreBackupLayers", Echomap.LogDebug, 'restoreBackupLayers', 'Done');
}
Echomap_LoaderCCG.prototype.clearLayers = function(actor,backupName){
	Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "clearLayers", Echomap.LogDebug, 'clearLayers', 'Called');
    if(!actor.composite)
        actor.composite = {};
    if(!actor.composite.layerdata)
    ...
    Echomap.Utils.log2("CCG", "Echomap_LoaderCCG", "clearLayers", Echomap.LogDebug, 'clearLayers', 'Done');
}
...
Echomap_CCG_ImgObject.prototype.initialize = function() {
	Echomap.Utils.log2("CCG", "CCG_ImgObject", "initialize", Echomap.LogDebug, 'Echomap_CCG_ImgObject', 'initialize' );
	_filename = undefined;
	_hue = 0;
};
...
Echomap_CCG_ImgObject.prototype.hue = function() {
	return Number( this._hue || 0 );
}
Echomap_CCG_ImgObject.prototype.toneR = function() {
	return Number( this._toneR || 0 );
}
Echomap_CCG_ImgObject.prototype.toneG = function() {
	return Number( this._toneG || 0 );
}
Echomap_CCG_ImgObject.prototype.toneB = function() {
	return Number( this._toneB || 0 );
}
//gray/grey
Echomap_CCG_ImgObject.prototype.toneY = function() {
	return this._toneY || 0 ;
}
...
//=============================================================================
// Echomap.CCGUtil
// The object for image processing utils
//=============================================================================
...
Echomap.CCGUtil.parseLayerNameToId = function(layerName,layertype) {
	...
	if(layertype==Echomap.CCGUtil.partytype.face){
		switch(layerName){
			...
			case 'hairfront1': 	layerId = 21; break;
		...
		switch(layerName){
			...
			case 'hairfront2': 	layerId = 13; break;
			case 'hairrear': 	layerId = 14; break;			
			case 'hairrear1': 	layerId = 14; break;
			case 'hairrear2': 	layerId = 15; break;
			case 'hairfront1': 	layerId = 16; break;
			case 'hairfront': 	layerId = 16; break;			
		 	case 'beastears': 	layerId = 17; break;
		 	case 'accb': 		layerId = 18; break;
		 	case 'acca': 		layerId = 19; break;
		 	case 'tail1': 		layerId = 20; break;
		 	case 'wings1':  	layerId = 21; break;
			case 'foreground':	layerId = 30; break;
		 	default: layerId = -1;
		}
	} else if (layertype==Echomap.CCGUtil.partytype.sv){
...
Echomap.CCGUtil.combineImagesLayer = function(bmp,imgLayer) {
	for (var i = 0, len = imgLayer.length; i < len; i++) {
		...
		var toneG = imgLocal.toneG();
		var toneB = imgLocal.toneB();
		var toneY = imgLocal.toneY();
		Echomap.Utils.log2("CCG", "CCGUtil", "combineImagesLayer", Echomap.LogDebug, 'combineImagesLayer', 'dpath', dpath, "Hue:" +hue+ " R:"+toneR+" G:"+toneG+" B:"+toneB);
...
Echomap_ProcessCCG.prototype.initialize = function() {
	Echomap.Utils.log2("CCG", "ProcessCCG", "initialize", Echomap.LogDebug, 'Echomap_ProcessCCG', 'initialize' );
};
Echomap_ProcessCCG.actorEquippedChanged = function(gameActor) {
	Echomap.Utils.log2("CCG", "ProcessCCG", "actorEquippedChanged", Echomap.LogDebug, 'actorEquippedChanged', 'Called' );
    Echomap.CCG.Echomap_Queue.queueImageCreateForActor(gameActor,Echomap.CCGUtil.partytype.face);
    Echomap.CCG.Echomap_Queue.queueImageCreateForActor(gameActor,Echomap.CCGUtil.partytype.tv);
    Echomap.CCG.Echomap_Queue.queueImageCreateForActor(gameActor,Echomap.CCGUtil.partytype.sv);
    Echomap.Utils.log2("CCG", "ProcessCCG", "actorEquippedChanged", Echomap.LogDebug, 'actorEquippedChanged', 'Done' );
};
...
Echomap_ProcessCCG.clearLayerFromActor = function(actorId,layerName,fileName,layerType,procNow) {
    var loaderProc = new Echomap_LoaderCCG();
    loaderProc.clearLayerFromActor(actorId,layerName,fileName,layerType,procNow);
}
Echomap_ProcessCCG.setLayerToActor = function( vparams ) {
    var loaderProc = new Echomap_LoaderCCG();
    loaderProc.setLayerToActor(vparams);
}
...
Echomap_ProcessCCG.processImageChange = function(actorId,parttype) {
    Echomap.Utils.log2("CCG", "ProcessCCG", "processImageChange", Echomap.LogDebug, 'processImageChange','Called');
	var gameActor = $gameActors.actor(actorId);
	...
	Echomap.Utils.log2("CCG", "ProcessCCG", "processImageChange", Echomap.LogDebug, 'processImageChange','Done');
}

Echomap_ProcessCCG.changeFaceLayer = function(actorId,layerName,layerData,keepLayer,procNow) {
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeFaceLayer", Echomap.LogDebug, 'changeFaceLayer','Called');
    ...
    if(procNow)
        Echomap_ProcessCCG.processImageChange(actor);
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeFaceLayer", Echomap.LogDebug, 'changeFaceLayer','Done');
};

Echomap_ProcessCCG.changeTVLayer = function(actorId,layerName,layerData,keepLayer,procNow) {
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeTVLayer", Echomap.LogDebug, 'changeTVLayer','Called');
	var actor   = $gameActors.actor(actorId)
	//var actorId = actor.actorId();
	if(!keepLayer)
    ...
    if(procNow)
        Echomap_ProcessCCG.processImageChange(actor);
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeTVLayer", Echomap.LogDebug, 'changeTVLayer','Done');
};

Echomap_ProcessCCG.changeSVLayer = function(actorId,layerName,layerData,keepLayer,procNow) {
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeSVLayer", Echomap.LogDebug, 'changeSVLayer','Called');
	var actor   = $gameActors.actor(actorId)
	//var actorId = actor.actorId();
	if(!keepLayer)
    ...
    if(procNow)
        Echomap_ProcessCCG.processImageChange(actor);
	Echomap.Utils.log2("CCG", "ProcessCCG", "changeSVLayer", Echomap.LogDebug, 'changeSVLayer','Done');
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === 'addLayerToActor') {
		...
		var argslice = undefined
		if( args.length>4){
			argslice = args.slice(4,args.length).join(" ");
		}
        Echomap_ProcessCCG.addLayerToActor(args[0],args[1],args[2],args[3],argslice);
        /*TODO
		var process = new Echomap_ProcessCCG();
    ...
    if (command === 'setLayerToActor') {
		...
		//Echomap_ProcessCCG.clearLayerFromActor(args[0],args[1],args[3]);
        //Echomap_ProcessCCG.addLayerToActor(args[0],args[1],args[2],args[3],argslice);
		//args[0],args[1],args[2],args[3],argslice
		var params = new Map();
		params.set("actorId",args[0])
		params.set("layerName",args[1])
		params.set("fileName",args[2])
		params.set("layerType",args[3])
		params.set("extradata",argslice)
		params.set("replace",true)
		Echomap_ProcessCCG.setLayerToActor(params);
        /*TODO
		var process = new Echomap_ProcessCCG();
		process.addLayerToActor(args[0],args[1],args[2],args[3]);
		process.processImageChange(actor);
